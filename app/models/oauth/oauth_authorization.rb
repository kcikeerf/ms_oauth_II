class Oauth::OauthAuthorization
  ...
  private
    ...
    def random_code
      self.code = SecureRandom.hex(SwtkOauthConfig.settings["random_length"])
    end

    # expiration time
    def create_expiration
      self.expire_at = Chronic.parse("in #{SwtkOauthConfig.settings["authorization_expires_in"]} seconds")
    end
end
